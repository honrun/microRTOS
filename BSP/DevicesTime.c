...
/* 系统运行时间，不可更改 */
volatile int64_t g_lTimeBase = 0;
/* 时区：默认东八区 */
volatile float g_fTimeUTC = 8.0f;


/* 判断是否为闰年 */
#define YEAR_LEAP(year) ((((year) & 3)      != 0) ? 0 : \
                         (((year) % 100)    != 0) ? 1 : \
                         (((year) % 400)    != 0) ? 0 : \
                         (((year) % 3200)   != 0) ? 1 : \
                         (((year) % 172800) != 0) ? 0 : 1)
...
uint8_t cTimeToWeek(int32_t iYear, int8_t cMonth, int8_t cDay)
{
    ...
    cWeek = ((iCentury / 4) - (iCentury * 2) + iYear + (iYear / 4) + (13 * (cMonth + 1) / 5) + cDay - 1) % 7;
...
/*
* Return:      void
* Parameters:  ptypeTime:时间结构体指针; lStamp: UNIX时间戳; cUTC: 时区（东时区为正、西时区为负）
...
int64_t lTimebaseGet(void)
{
    ...
    do
    {
        lTimeBase = g_lTimeBase;

        uiTimeLow = TIMER_CNT(TIMER5);
    }
    while(lTimeBase != g_lTimeBase);

    return lTimeBase + uiTimeLow;
}

/* 设置时间戳：单位us */
void vTimestampSet(int64_t lUNIXTimestamp)
{
    g_lTimestamp = lUNIXTimestamp - lTimebaseGet();
}

/* 获取时间戳：单位us */
int64_t lTimestampGet(void)
{
    return g_lTimestamp + lTimebaseGet();
}
...
char *pcStampToTimeStrings(int64_t lUNIXTimestamp)
{
    ...
    /* 东8区 */
    vStampToTime(&typeTime, lUNIXTimestamp, 8);
